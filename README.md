...
:---: | :---:
![](https://yt3.ggpht.com/ytc/AAUvwngGonLpA32yyhoRXCSn-JcWHTKlf2ey9D3tZ8XYww=s88-c-k-c0x00ffffff-no-rj) | The Discord Channel, subscribe for Discord Tips and Tricks, server decoration, bot setup tutorials and more to spice up your Discord Experience! Click [here](https://www.youtube.com/techiegaurav) to subscribe!
...
| **💪 Mass Ping**   | Made for Mass Shiny Hunt Tag. People who have `Sh POKEMONNAME` in server-nickname will get pinged if any `POKEMONNAME` spawns.  `Quick Find` feature should be enabled for better performance.   | From https://replit.com/repls, open your bot Repl, go to `Secrets (Environment Variables)` create a Secret with Key `PING` and Value will be `True`. Click `Save`. Wait for it to restart after 30 minutes. To disable, delete the secret called `PING` or set the value to `False`. |
| **🚫 Anti Snipe**   | The bot will warn the user if they are caught Sniping. (Snipers catch Pokémons but do not help Pokémons spawn by sending messages) | From https://replit.com/repls, open your bot Repl, go to `Secrets (Environment Variables)` create a Secret with Key `SNIPE` and Value will be `True`. Click `Save`. Wait for it to restart after 30 minutes. To disable, delete the secret called `SNIPE` or set the value to `False`.  |
